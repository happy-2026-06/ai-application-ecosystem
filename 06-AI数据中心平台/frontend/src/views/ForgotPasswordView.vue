...
async function handleForgot() { err.value = ''; loading.value = true; try { await authApi.forgotPassword(username.value.trim()); msg.info('演示模式：跳过验证'); step.value = 2 } catch (e: any) { err.value = e?.response?.data?.detail || '请求失败' } finally { loading.value = false } }
...
.fp-step.active { background: #0ea5e9; color: #fff; border-color: #0ea5e9; }
.fp-step.done { background: #0ea5e9; color: #fff; border-color: #0ea5e9; }
.fp-line { width: 48px; height: 2px; background: #e2e8f0; transition: background .25s; }
.fp-line.done { background: #0ea5e9; }
.fp-labels { display: flex; justify-content: space-between; padding: 0 6px; margin-bottom: 32px; font-size: 11px; color: #94a3b8; }
.fp-labels span.on { color: #0ea5e9; font-weight: 600; }
.fp-body { text-align: center; }
.fp-icon-wrap { width: 64px; height: 64px; line-height: 64px; border-radius: 20px; background: #f0f9ff; display: inline-block; font-size: 28px; margin-bottom: 16px; }
...
.demo-badge { background: #f0f9ff; color: #0ea5e9; font-size: 11px; font-weight: 700; padding: 2px 8px; border-radius: 4px; margin-right: 4px; }
.fp-input { margin-bottom: 14px; }
:deep(.fp-input .n-input) { --n-border: 1px solid #e2e8f0 !important; --n-border-focus: 1px solid #0ea5e9 !important; --n-color: #fff !important; --n-text-color: #334155 !important; --n-placeholder-color: #94a3b8 !important; border-radius: 12px !important; }
...
.fp-btn { height: 50px !important; font-size: 16px !important; font-weight: 700 !important; border-radius: 14px !important; background: linear-gradient(135deg, #0ea5e9, #0284c7) !important; border: none !important; box-shadow: 0 4px 16px rgba(14,165,233,.2); margin-top: 8px; }
.fp-btn:hover { transform: translateY(-1px); box-shadow: 0 8px 24px rgba(14,165,233,.35) !important; }
.fp-back { margin-top: 16px; font-size: 13px; color: #0ea5e9; cursor: pointer; }
