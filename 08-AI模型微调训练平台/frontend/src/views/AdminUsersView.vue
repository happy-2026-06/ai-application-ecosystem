<template>
  <div class="apage">
    <div class="atop">
      <h2>👥 用户管理</h2>
      <n-button type="primary" @click="$router.push('/admin/dashboard')" ghost>📊 仪表盘</n-button>
    </div>
...
const cols=[
  ...
  {title:'角色',key:'role',width:80,render(r:any){return h(NTag,{type:r.role==='admin'?'warning':'info',round:true,size:'small'},{default:()=>r.role==='admin'?'管理员':'用户'})}},
  {title:'状态',key:'is_active',width:80,render(r:any){return h(NTag,{type:r.is_active?'success':'error',round:true,size:'small'},{default:()=>r.is_active?'正常':'禁用'})}},
  {title:'操作',key:'act',width:80,render(r:any){return h(NPopconfirm,{onPositiveClick:()=>toggle(r)},{trigger:()=>h(NButton,{size:'small',type:r.is_active?'warning':'success'},{default:()=>r.is_active?'禁用':'启用'}),default:()=>`确认${r.is_active?'禁用':'启用'}用户 ${r.username}？`})}},
]
...
.atop h2{margin:0;font-size:22px;color:#0f172a}
