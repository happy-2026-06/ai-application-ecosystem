...
async def batch_delete_sessions(
    ...
    """Soft-delete multiple sessions at once."""
    session_ids = request.session_ids
    if not session_ids:
        raise HTTPException(status_code=400, detail="请提供要删除的会话ID列表")
    ...
    for s in sessions:
        s.status = "deleted"
    await db.flush()
    return {"deleted": len(sessions), "message": f"已删除 {len(sessions)} 个会话"}
...
async def ask_question(
    ...
    Rate limited to 10 requests/minute per IP (SSE connections are expensive).
    """
