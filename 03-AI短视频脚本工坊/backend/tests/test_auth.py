...
# Use the self-contained test password, NOT whatever .env says
_ADMIN_PWD = os.environ.get("ADMIN_PASSWORD", "test1234")
...
async def test_register_duplicate(client):
    await client.post("/api/auth/register", json={"username": "dup1", "password": "test1234"})
    resp = await client.post("/api/auth/register", json={"username": "dup1", "password": "test1234"})
...
async def test_login_admin(client):
    resp = await client.post("/api/auth/login", json={"username": "admin", "password": _ADMIN_PWD})
...
async def test_login_nonexistent(client):
    resp = await client.post("/api/auth/login", json={"username": "nobody", "password": "test1234"})
    assert resp.status_code == 401
...
async def test_change_password_ok(client, admin_headers):
    resp = await client.post("/api/auth/change-password", json={
        "old_password": _ADMIN_PWD, "new_password": "newpass123"
