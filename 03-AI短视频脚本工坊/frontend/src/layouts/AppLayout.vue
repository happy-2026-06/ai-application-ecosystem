<template>
  <div class="app-shell">
    <!-- Sidebar — Cinematic Dark -->
    <nav class="app-sidebar">
      <div class="sb-brand" @click="$router.push('/studio')">
        <span class="sb-logo">🎬</span>
        <span class="sb-name">脚本工坊</span>
      </div>

      <div class="sb-nav">
        <div
          v-for="item in navItems"
          :key="item.key"
          class="sb-item"
          :class="{ active: currentRoute === item.key }"
          @click="go(item.key)"
          :title="item.label"
        >
          <span class="sb-icon">{{ item.icon }}</span>
          <span class="sb-label">{{ item.label }}</span>
        </div>
      </div>

      <div class="sb-spacer" />

      <div class="sb-bottom-nav">
        <div class="sb-item" @click="authStore.toggleDarkMode()">
          <span class="sb-icon">{{ authStore.isDarkMode ? '☀️' : '🌙' }}</span>
          <span class="sb-label">{{ authStore.isDarkMode ? '浅色模式' : '深色模式' }}</span>
        </div>
        <div class="sb-item" :class="{ active: currentRoute === 'settings' }" @click="go('settings')">
          <span class="sb-icon">👤</span>
          <span class="sb-label">个人中心</span>
        </div>
        <div class="sb-item" @click="handleLogout">
          <span class="sb-icon">🚪</span>
          <span class="sb-label">退出</span>
        </div>
      </div>

      <div class="sb-user" @click="go('settings')">
        <span class="sb-avatar">{{ authStore.avatar }}</span>
        <div class="sb-user-info">
          <div class="sb-username">{{ authStore.user?.display_name || authStore.user?.username }}</div>
          <div class="sb-role">{{ authStore.isAdmin ? '导演' : '创作者' }}</div>
        </div>
      </div>
    </nav>

    <!-- Main Content -->
    <main class="app-main">
      <router-view />
    </main>
  </div>
</template>

<script setup lang="ts">
import { computed } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { useAuthStore } from '../stores/auth'

const route = useRoute()
const router = useRouter()
const authStore = useAuthStore()

const currentRoute = computed(() => {
  const name = String(route.name || '')
  if (name.startsWith('Admin')) return 'admin'
  if (name === 'Settings') return 'settings'
  if (name === 'ScriptStudio' || name === 'NotFound') return 'studio'
  return 'studio'
})

const navItems = computed(() => {
  const items: { key: string; icon: string; label: string }[] = [
    { key: 'studio', icon: '🎬', label: '脚本工坊' },
  ]
  if (authStore.isAdmin) {
    items.push({ key: 'admin', icon: '⚙️', label: '后台管理' })
  }
  return items
})

function go(key: string) {
  if (key === 'studio') router.push('/studio')
  else if (key === 'admin') router.push('/admin/dashboard')
  else if (key === 'settings') router.push('/settings')
}

function handleLogout() {
  authStore.logout()
  router.push('/login')
}
</script>

<style scoped>
.app-shell { display: flex; height: 100vh; overflow: hidden; }

/* ═══ Sidebar — Cinematic Dark ═══ */
.app-sidebar {
  width: 220px; min-width: 220px;
  display: flex; flex-direction: column;
  background: linear-gradient(180deg, #1A1A24 0%, #14141C 100%);
  color: #d0d0d8;
  user-select: none;
}

/* Brand */
.sb-brand {
  display: flex; align-items: center; gap: 10px;
  padding: 22px 18px 18px; cursor: pointer;
  border-bottom: 1px solid rgba(200,169,81,.12);
}
.sb-logo { font-size: 28px; }
.sb-name {
  font-size: 17px; font-weight: 700; letter-spacing: 0.5px;
  background: linear-gradient(90deg, #E8C860, #C8A951);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  background-clip: text;
}

/* Nav items */
.sb-nav { padding: 8px 10px; flex-shrink: 0; }
.sb-item {
  display: flex; align-items: center; gap: 10px;
  padding: 10px 12px; border-radius: 10px;
  cursor: pointer; transition: all .15s;
  margin-bottom: 2px; color: #a0a0b0;
  border-left: 3px solid transparent;
}
.sb-item:hover {
  background: rgba(200,169,81,.08); color: #e0d0b0;
}
.sb-item.active {
  background: rgba(200,169,81,.12); color: #fff;
  border-left-color: #C8A951;
}
.sb-icon { font-size: 20px; width: 28px; text-align: center; flex-shrink: 0; }
.sb-label { font-size: 14px; font-weight: 500; white-space: nowrap; }

.sb-spacer { flex: 1; }
.sb-bottom-nav { padding: 0 10px 8px; }

/* User area */
.sb-user {
  display: flex; align-items: center; gap: 10px;
  padding: 14px 16px; cursor: pointer;
  border-top: 1px solid rgba(200,169,81,.12);
  margin: 0 8px;
}
.sb-avatar { font-size: 28px; }
.sb-user-info { flex: 1; min-width: 0; }
.sb-username {
  font-size: 13px; font-weight: 600; color: #e8e0c8;
  overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
}
.sb-role { font-size: 11px; color: #a09870; margin-top: 1px; }

/* ═══ Main Content ═══ */
.app-main {
  flex: 1; min-width: 0;
  background: var(--bg-body, #FAF9F4);
  overflow: hidden;
  transition: background .3s;
}

/* ═══ Dark Mode ═══ */
[data-theme="dark"] .app-sidebar {
  background: linear-gradient(180deg, #0D0D18 0%, #0A0A12 100%);
}
[data-theme="dark"] .sb-brand { border-bottom-color: rgba(200,169,81,.08); }
[data-theme="dark"] .sb-item:hover { background: rgba(200,169,81,.06); }
[data-theme="dark"] .sb-item.active { background: rgba(200,169,81,.1); }
[data-theme="dark"] .sb-user { border-top-color: rgba(200,169,81,.08); }
[data-theme="dark"] .sb-username { color: #e0d8b8; }
[data-theme="dark"] .sb-role { color: #887840; }
[data-theme="dark"] .app-main { background: #0D0D14; }
</style>
