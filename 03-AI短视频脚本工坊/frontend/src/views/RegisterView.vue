  ...
  <div class="reg-page">
    ...
    <div class="reg-left">
      <div class="left-inner">
        ...
        <h1 class="anim-fade-in-up">加入脚本工坊</h1>
        <p class="anim-fade-in-up anim-delay-1">注册即可使用 AI 分镜脚本生成</p>
        <div class="feat-list">
          <div class="feat anim-fade-in-up anim-delay-1"><span>🎬</span> 专业分镜</div>
          <div class="feat anim-fade-in-up anim-delay-2"><span>🎤</span> AI配音</div>
          <div class="feat anim-fade-in-up anim-delay-3"><span>📝</span> 字幕导出</div>
          <div class="feat anim-fade-in-up anim-delay-4"><span>🚀</span> 秒级生成</div>
        </div>
      </div>
    </div>
        ...
        <n-form ref="fr" :model="f" :rules="r" label-placement="top">
          ...
          <n-form-item label="密码" path="p">
            <n-input
              ...
              placeholder="至少6位，建议包含字母+数字+符号"
              size="large"
            >
              <template #prefix><span class="input-prefix-icon">🔒</span></template>
              <template #suffix>
                <button type="button" class="pwd-toggle" @click="showPwd = !showPwd" :title="showPwd ? '隐藏' : '显示'">
                  {{ showPwd ? '🙈' : '👁️' }}
                </button>
              </template>
            </n-input>
            ...
            <div v-if="f.p" class="pw-strength">
              ...
              <span class="pw-tip" v-if="pwStrength.level < 3">{{ pwStrength.tip }}</span>
            </div>
          </n-form-item>
          ...
          <n-form-item label="确认密码" path="cp">
            <n-input v-model:value="f.cp" type="password" placeholder="再次输入密码" size="large">
              <template #prefix><span class="input-prefix-icon">🔒</span></template>
              <template #suffix>
                <span v-if="f.cp && f.p" class="pw-match" :class="{ match: f.cp === f.p, mismatch: f.cp !== f.p }">
                  {{ f.cp === f.p ? '✅' : '❌' }}
                </span>
              </template>
            </n-input>
          ...
          <n-form-item path="agree">
            <n-checkbox v-model:checked="f.agree">
              我已阅读并同意
              <n-button text type="primary" size="tiny">服务条款</n-button>
              和
...
const r: FormRules = {
  ...
  cp: [
    ...
    {
      validator: (_: any, v: string) => v === f.p,
      message: '两次密码不一致',
      trigger: 'blur',
    },
  ],
...
const pwStrength = computed(() => {
  const p = f.p
  if (!p) return { level: 0, label: '', color: '', style: { width: '0%' }, tip: '' }
  ...
  if (score <= 2) {
    return {
      ...
      tip: '建议使用8位以上字母+数字+符号的组合',
    }
  }
  if (score <= 4) {
    return {
      level: 2, label: '中等', color: '#F59E0B',
      style: { width: '55%', background: '#F59E0B' },
      tip: '再添加大小写字母和符号会更安全',
    }
  }
  return {
    level: 3, label: '强', color: '#10B981',
...
.reg-left {
  flex: 1;
  background: linear-gradient(135deg, #1A1A24 0%, #2A2A38 35%, #3A3020 100%);
  display: flex;
  align-items: center;
...
.reg-left::before {
  ...
  width: 300px;
  height: 300px;
  background: radial-gradient(circle, rgba(200,169,81,.12), transparent 70%);
  border-radius: 50%;
...
.reg-right {
  ...
  padding: 40px;
  background:
    radial-gradient(ellipse at 20% 50%, rgba(200,169,81,.03) 0%, transparent 50%),
    radial-gradient(ellipse at 80% 20%, rgba(200,169,81,.02) 0%, transparent 50%),
    #fff;
  position: relative;
  z-index: 1;
}
...
.reg-btn:hover {
  ...
  box-shadow: 0 8px 24px rgba(200,169,81,.35) !important;
}
...
/* ═══════════════════════════════════════════════════════════════════════════
   Dark Mode
   ═══════════════════════════════════════════════════════════════════════════ */
[data-theme="dark"] .reg-right {
  background:
    radial-gradient(ellipse at 20% 50%, rgba(200,169,81,.04) 0%, transparent 50%),
    radial-gradient(ellipse at 80% 20%, rgba(200,169,81,.03) 0%, transparent 50%),
    #0D0D14;
}

[data-theme="dark"] .reg-left {
  background: linear-gradient(135deg, #0A0A12 0%, #14141C 35%, #1E1A12 100%);
}

[data-theme="dark"] .left-inner > p {
  color: #8888a0;
}
...
@media (max-width: 768px) {
  ...
  .reg-left { flex: none; padding: 36px 24px; }
  .reg-right { width: 100%; padding: 20px; }
