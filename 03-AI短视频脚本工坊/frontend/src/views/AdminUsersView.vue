...
const cols = [
  ...
  { title: '邮箱', key: 'email', ellipsis: { tooltip: true } },
  {
    ...
    render(r: any) {
      return h(NTag, { type: r.role === 'admin' ? 'warning' : 'info' }, { default: () => r.role === 'admin' ? '管理员' : '用户' })
    }
  },
  {
  ...
  },
  {
    ...
    render(r: any) {
      return h(NButton, {
        size: 'small', type: r.is_active ? 'error' : 'success',
        onClick: () => toggle(r),
...
/* Dark mode */
[data-theme="dark"] :deep(.n-data-table) { color: #ddd; }
[data-theme="dark"] :deep(.n-data-table-th) { background: #1A1A25; color: #ddd; }
[data-theme="dark"] :deep(.n-data-table-td) { background: #14141E; color: #d0d0d8; border-bottom-color: #2A2A35; }
</style>
