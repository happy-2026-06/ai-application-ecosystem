<template>
  <div class="apage">
    ...
    <n-grid cols="3" x-gap="12" style="margin-bottom:20px;">
      <n-gi><n-card size="small" title="训练统计"><n-statistic label="已完成会话" :value="dash?.completed_sessions||0" /><n-statistic label="平均分" :value="dash?.avg_score||0" /></n-card></n-gi>
...
const cards=computed(()=>[{label:'总用户',value:dash.value?.total_users||0},{label:'活跃用户',value:dash.value?.active_users||0},{label:'训练会话',value:dash.value?.total_training_sessions||0},{label:'训练轮数',value:dash.value?.total_training_rounds||0}])
...
.atop { margin-bottom:20px; } .atop h2 { margin:0; font-size:22px; }
</style>
