...
class Settings(BaseSettings):
    ...
    SECRET_KEY: str = Field(
        default="rag-cs-dev-secret-must-override-in-prod-4a1f",
    ...
    JWT_SECRET_KEY: str = Field(
        default="rag-cs-jwt-dev-key-must-override-in-prod-6d3c",
    ...
    CORS_ORIGINS: List[str] = Field(
        default=["http://localhost:3001", "http://localhost:80", "http://localhost"],
    )

    # ── Pre-seeded admin account ─────────────────────────────────────
    ADMIN_USERNAME: str = "admin"
    ADMIN_PASSWORD: str = Field(
        default="RAG-cs-admin-2024!",
    ...
    def _warn_dev_defaults(cls, v: str, info) -> str:
        ...
        dev_markers = ("change-me", "dev-secret", "dev-password", "admin123",
                       "neo4j-dev", "jwt-dev", "must-override", "RAG-cs")
