...
def main():
    ...
    subprocess.Popen(
        [sys.executable, "-m", "uvicorn", "app.main:app", "--host", "0.0.0.0", "--port", "8202"],
        cwd=BACKEND,
    ...
    if sys.platform == "win32":
        subprocess.Popen(
            ["cmd", "/c", "npx", "vite", "--host", "0.0.0.0", "--port", "3002"],
            cwd=FRONTEND,
            creationflags=creationflags,
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
    ...
    for i in range(45):
        ...
        try:
            s = socket.socket()
            s.settimeout(1)
            if s.connect_ex(("127.0.0.1", 8202)) == 0:
                s.close()
                print()
                print("=" * 45)
                print("  启动成功!")
                print("  前端: http://127.0.0.1:3002")
                print("  后端: http://127.0.0.1:8202/docs")
                print("=" * 45)
                print()

                if sys.platform == "win32":
                    os.startfile("http://127.0.0.1:3002")
                break
            s.close()
        except Exception:
            pass
