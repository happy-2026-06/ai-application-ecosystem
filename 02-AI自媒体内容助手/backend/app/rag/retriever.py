...
async def _bm25_search(query: str, top_k: int = 20) -> list[dict]:
    """BM25 keyword-based retrieval (placeholder).

    In production, this uses a BM25 index built from all documents.
    For the initial implementation, this returns an empty list.
    """
    # TODO: Implement BM25 index using rank_bm25 or similar
    return []


async def _graph_search(query: str, top_k: int = 20) -> list[dict]:
    """Neo4j graph-based retrieval (GraphRAG).

    Converts natural language query to Cypher and queries Neo4j.
    For the initial implementation, this returns an empty list.
    """
    # TODO: Implement GraphRAG with Neo4j
    # from app.rag.graph.retriever import graph_retrieve
    # return await graph_retrieve(query, top_k)
    return []
